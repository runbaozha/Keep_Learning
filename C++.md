...
- **static**
  ...
  - static对于函数
       ...
       作用域限定：尽管static函数的作用域限定在当前文件，但这并不意味着不能在头文件中定义static函数。实际上，只要在包含该头文件的每个源文件中都有唯一的函数定义，就可以在头文件中定义static函数。

       链接问题：需要注意的是，如果在头文件中定义了一个static函数，并且在多个源文件中包含了该头文件，那么在链接阶段可能会出现重定义的错误。为了避免这种情况，可以在头文件中只做声明，而在某个源文件中做定义。
  ...
  - static对于类
    ...
    1. 成员变量
       ...
       因此，**static成员必须在类外进行初始化，而不能在构造函数内进行初始化。不过也可以用const修饰static数据成员在类内初始化 。** （参考test_static.cc）
    ...
    2. 成员函数
       ...
       **不可以同时用const和static修饰成员函数。** （参考test_staticmethod.cc）
...
局部静态变量位于内存中的静态存储区，未经初始化的局部静态变量会自动初始化为0。但是局部静态变量的作用域还是局部作用域，定义它的函数或者代码块结束的时候，作用域也随之结束。但是该变量值不会被销毁，而是在内存中驻留下来，知道程序全部结束，这个驻留的值我们不能访问它。
...
inline 允许函数在多个编译单元中存在。inline函数在每个包含它的源文件中都有独立的定义。因此可以在头文件中定义一个inline函数，然后在多个源文件中包含这个头文件。
如果一个函数在头文件中被定义，但没有用inline声明，此时如果有多个源文件包含该头文件，就会报 multiple define的错误，这是因为C++有one definition rule的规则。


目前所有主流编译器都会忽略inline的指定（或建议）生成内联函数这个原始含义，编译器有自己的一套方法决定是否内联一个函数，完全无视程序员写的inline。
...
2. 让父类同名函数在子类中以重载方式使用或者是在子类私有继承父类的时候使用，这样派生类的对象就可以访问基类的 proteced 成员变量和 public 成员函数了。
```c++
...
我们知道C++的多态性（运行时）是由虚函数实现的，对于多态性的对象，无法在程序编译阶段确定对象的类型。当类中含有虚函数时，其基类的指针就可以指向任何派生类的对象，这时就有可能不知道基类指针到底指向的是哪个对象的情况，类型的确定要在运行时利用运行时类型标识做出。为了获得一个对象的类型可以使用typeid函数，该函数返回一个对type_info类对象的引用，要使用typeid必须使用头文件`<typeinfo>`，因为typeid是一个返回类型为typ_info的引用的函数
...
上述就是菱形继承会产生两个问题。首先是空间问题，不节省空间。类B中有类A的数据，类C中也有。其次是二义性，歧义问题。当类D调用类A中的数据时候，到底是走类B的还是走类C的？如下：
...
第一种：8，12   第二种：8，8   第三种：8，16   第四种：8，8

每个存在虚函数的类都要有一个8字节的虚表指针指向自己的虚函数表，所以每种情况的类a所占的字节数应该是没有什么问题的(指针所占字节数跟平台有关，64位的平台是8个字节。32位的4个字节)。同时虚继承要有这样的一个指针vptr_b_a，这个指针叫虚类指针，也是四个字节；还要包括类a的字节数，所以类b的字节数就求出来了。而“第二种”和“第四种”情况则不包括vptr_b_a这个指针，这回应该木有问题了吧。
...
//第⼆种：全局变量的初始化在程序初始阶段，先于 main 函数的执⾏
int test1(){
...
vector变量本身作为局部变量的话存放在栈区中，作为静态变量和全局变量的话则是存放在静态区中。

至于 vector里的元素一般是在堆中 ，但根据Allocator的实现也有可能是在其他地方，比如栈或者静态区。
...
采用一块所谓的map作为主控，这里的map实际上是一块大小连续的空间，其中每一个元素我们称之为节点node，都指向了另一段连续线性空间，成为缓冲区，缓冲区才是deque的真正存储空间主体。
