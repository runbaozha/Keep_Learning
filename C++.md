...
。在调用内联函数时，要保证内联函数的定义让编译器看到，也就是说，内联函数inline必须要定义在头文件中，这与通常的函数定义是不一样的。如果要强制编译器做内联，在GCC或者Clang里可以用：
...
```
inline 还可以用于控制链接。inline函数在每个包含它的源文件中都有独立的定义。因此可以在头文件中定义一个inline函数，然后在多个源文件中包含这个头文件。
